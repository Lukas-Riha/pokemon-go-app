...
(() => {
 ...
 window.AtlasEnhanceDetail=(container,row,c)=>{
  old(container,row,c);$('.atlas-drawer-header .hra-pruh')?.remove();const bar=container.querySelector('.hra-pruh');if(bar){bar.setAttribute('aria-label','Zapsat změnu ve hře');bar.querySelectorAll('[aria-pressed]').forEach(b=>b.removeAttribute('aria-pressed'));$('.atlas-drawer-header').append(bar)}
  const sub=document.createElement('p');sub.className='atlas-role-summary';sub.textContent=c.keepSub||'';if(sub.textContent)container.querySelector('.atlas-verdict-first')?.append(sub);
 };
