...
(() => {
  ...
  function rowHTML(r){const c=cache.computed[r.id]||{},iv=c.ivPct;return `<button class="atlas-row" data-atlas-detail="${esc(r.id)}" aria-label="Detail ${esc(r.pokemon)}"><span class="atlas-poke">${monImage(r)}<span><b>${esc(r.pokemon)}${r.star?' <span aria-label="Označeno">★</span>':''}${window.AtlasTags(c,r)}</b><small><span class="atlas-level">L${esc(r.level||'?')}</span><span>${iv==null?'IV neznámé':Math.round(iv*100)+' % IV'}</span></small></span></span><span class="atlas-num">${fmt(r.cp)}<small>CP</small></span><span class="atlas-decision"><span class="atlas-pill" data-verdict="${window.AtlasVerdict(c).tone}">${decision(c)}</span>${(P.atlasDuvody&&P.atlasDuvody(c))||'<small>'+esc(role(c))+'</small>'}</span><span class="atlas-next">${window.AtlasJourneyHTML(c,r)}</span><span class="atlas-arrow">${icon('next')}</span></button>`;}
...
(() => {
  ...
  window.AtlasEditRow=(id,field)=>{
    ...
    // Útoky se nabízejí z toho, co se druh může naučit (engine utokyDruhu); při změně druhu se nabídka přepočítá.
    const naplnUtoky=()=>{if(!P.utokyDruhu)return;const u=P.utokyDruhu(form.elements.pokemon.value);const dl=(id,jmena)=>{let d=form.querySelector('#'+id);if(!d){d=document.createElement('datalist');d.id=id;form.append(d)}d.innerHTML=jmena.map(j=>`<option value="${esc(j)}"></option>`).join('')};dl('atlasFastList',u.fast);dl('atlasChargedList',u.charged);form.elements.fastMove.setAttribute('list','atlasFastList');form.elements.charged1.setAttribute('list','atlasChargedList');form.elements.charged2.setAttribute('list','atlasChargedList')};naplnUtoky();form.elements.pokemon.addEventListener('change',naplnUtoky);
...
(() => {
 ...
 window.AtlasEnhanceDetail=(container,row,c)=>{
  old(container,row,c);$('.atlas-drawer-header .hra-pruh')?.remove();const bar=container.querySelector('.hra-pruh');if(bar){bar.setAttribute('aria-label','Zapsat změnu ve hře');bar.querySelectorAll('[aria-pressed]').forEach(b=>b.removeAttribute('aria-pressed'));const editBtn=$('#atlasEditPokemon');if(editBtn)editBtn.before(bar);else $('.atlas-drawer-header').append(bar)}
  ...
  const why=container.querySelector('.atlas-verdict-first .d-why');if(why&&container.querySelector('.atlas-verdict-first .d-duvody')){const hlava=container.querySelector('.atlas-verdict-first .d-verdict>b');if(hlava&&why.textContent.trim())hlava.setAttribute('data-tip',why.textContent.trim());why.remove();}
 };
...
(() => {
 ...
 function card(e){const a=date(e[3]),b=end(e[4]),label=!a||!b?'Termín nepotvrzen':live(e)?'Probíhá':a>new Date()?'Chystá se':'Ukončeno';return `<button class="atlas-event-card" data-event-index="${events.indexOf(e)}"><span class="atlas-event-status">${label}</span><div class="atlas-event-art">${[...new Set((e[7]||[]).flatMap(w=>((w[2]?.raid?.length?w[2].raid:w[2]?.spawn)||[]).filter(x=>x[1]!==-1).map(x=>x[0])).concat((e[6]||[]).filter(x=>x[1]!==-1).map(x=>x[0])))].slice(0,3).map(image).join('')}</div><h3>${esc(e[0])}</h3><p>${format(e[3])} → ${format(e[4])}</p><span class="atlas-event-link">Podrobnosti akce →</span></button>`}
 ...
 const shown=events.filter(e=>overlap(e,start,finish)&&!['season','raid-battles'].includes(e[1])).sort((a,b)=>(+end(a[4])<Date.now())-(+end(b[4])<Date.now())||+end(a[4])-+end(b[4])||String(a[0]).localeCompare(b[0]));
 ...
 root.innerHTML=`<div class="atlas-overview"><div class="atlas-overview-intro"><span class="atlas-feed-note">Data: ${esc(stamp||'datum neznámé')}${stale?' · Aktualitu ověř ve zdroji':''}</span></div><section class="atlas-panel atlas-overview-calendar"><div class="atlas-panel-head"><h2>Dnes a brzy</h2><div><button class="atlas-mini-btn" data-overview-week="-7" aria-label="Předchozí týden">←</button> <button class="atlas-mini-btn" data-overview-today>Dnes</button> <button class="atlas-mini-btn" data-overview-week="7" aria-label="Další týden">→</button></div></div><div class="atlas-week">${days}</div><div class="atlas-event-grid">${shown.slice(0,6).map(card).join('')||'<p class="atlas-empty">Pro tento den nemáme potvrzenou akci.</p>'}</div>${shown.length>6?`<p class="atlas-small-note">${(n=>n===1?'Další 1 akci':n<=4?'Další '+n+' akce':'Dalších '+n+' akcí')(shown.length-6)} najdeš v úplném kalendáři.</p>`:''}<button class="atlas-mini-btn" data-atlas-nav="events">Celý kalendář →</button></section><div class="atlas-battle-columns">${[[raids,'Raidy'],[max,'Max souboje']].map(([es,title])=>`<section class="atlas-panel"><div class="atlas-panel-head"><h2>${title}</h2></div>${title==='Max souboje'&&!maxLive.length&&max.length?'<p class="atlas-small-note">Nejbližší potvrzená akce · pro dnešek nemáme potvrzený seznam bossů.</p>':''}<div class="atlas-event-grid">${es.map(card).join('')||'<p class="atlas-empty">V datech není potvrzené aktuální okno. To neznamená, že žádné souboje neběží.</p>'}</div><p class="atlas-small-note">Dostupnost podle termínů akcí${stale?', data mohou být zastaralá':''}. Sestavu ověř v Taháku.</p><button class="atlas-mini-btn" data-overview-teams>Otevřít Tahák →</button></section>`).join('')}</div><section class="atlas-panel"><div class="atlas-panel-head"><h2>Spawny probíhajících akcí</h2></div><p class="atlas-small-note">Platí časová a další omezení u jednotlivých skupin. Nejde o živou mapu okolí.</p>${spawns.map(({e,w})=>`<details class="atlas-spawn-preview" open><summary>${esc(e[0])}</summary>${list(w[2].spawn,'spawn')}<button class="atlas-mini-btn" data-event-index="${events.indexOf(e)}">Podmínky a další obsah →</button></details>`).join('')||'<p class="atlas-empty">Nemáme potvrzený seznam spawnů pro právě probíhající časové okno.</p>'}</section><section class="atlas-panel"><div class="atlas-panel-head"><div><h2>Co se vyplatí připravit</h2><p>První kroky v pořadí enginu. Dostupnost prachu a bonbónů si ověř.</p></div><button class="atlas-mini-btn" data-atlas-nav="invest">Celý plán →</button></div><div class="atlas-plan-preview">${plans.map(p=>`<button class="atlas-plan-card" data-atlas-detail="${esc(p.row.id)}">${image(p.row.pokemon)}<div><h3>${esc(p.row.pokemon)}</h3><p>${esc(p.role||c[p.row.id]?.keepSub)}</p><b>${p.evoluce?'Evoluce a další krok':'Vylepšení'}${p.cilJmeno?' → '+esc(p.cilJmeno):''}</b><div>${window.AtlasJourneyHTML(c[p.row.id],p.row)}</div></div></button>`).join('')||'<p class="atlas-empty">Engine nyní nenabízí ověřený investiční krok.</p>'}</div></section><section class="atlas-panel"><div class="atlas-panel-head"><h2>Před hraním</h2></div><div class="atlas-preflight"><button class="atlas-mini-btn" data-click="boxModeBtn">Projít box</button><button class="atlas-mini-btn" data-atlas-filter="missing">Doplnit útoky · ${missing} kusů</button><button class="atlas-mini-btn" data-atlas-nav="events">Ověřit termíny a bonusy</button></div></section></div>`;
